...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
        ...
        <xsl:choose>
            <xsl:when test="contains($newStyle, ' ')">
                <xsl:call-template name="styleAlign">
                    <xsl:with-param name="reversedStyles"
                                    select="concat($reversedStyles, ' ', substring-before($newStyle, ' '))"/>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="ancestor::*/tt:head/tt:styling/tt:style[@xml:id = $newStyle]/@tts:textAlign != ''">
                        <xsl:value-of
                                select="ancestor::*/tt:head/tt:styling/tt:style[@xml:id = $newStyle]/@tts:textAlign"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:choose>
                            <xsl:when test="$reversedStyles != ''">
                                <xsl:call-template name="styleAlign">
                                    <xsl:with-param name="reversedStyles"
                                                    select="substring-after($reversedStyles, ' ')"/>
    ...
    <xsl:template name="translateHexvalue">
        ...
        <xsl:choose>
            <xsl:when
                    test="number(substring($color, 2, 1)) &lt;= 7 and number(substring($color, 4, 1)) &lt;= 7 and number(substring($color, 6, 1)) &lt;= 7">
                <xsl:value-of select="'textBlack'"/>
            </xsl:when>
            <xsl:when
                    test="number(substring($color, 2, 1)) &lt;= 7 and number(substring($color, 4, 1)) &lt;= 7 and not(number(substring($color, 6, 1)) &lt;= 7)">
                <xsl:value-of select="'textBlue'"/>
            </xsl:when>
            <xsl:when
                    test="number(substring($color, 2, 1)) &lt;= 7 and not(number(substring($color, 4, 1)) &lt;= 7) and number(substring($color, 6, 1)) &lt;= 7">
                <xsl:value-of select="'textGreen'"/>
            </xsl:when>
            <xsl:when
                    test="number(substring($color, 2, 1)) &lt;= 7 and not(number(substring($color, 4, 1)) &lt;= 7) and not(number(substring($color, 6, 1)) &lt;= 7)">
                <xsl:value-of select="'textCyan'"/>
            </xsl:when>
            <xsl:when
                    test="not(number(substring($color, 2, 1)) &lt;= 7) and number(substring($color, 4, 1)) &lt;= 7 and number(substring($color, 6, 1)) &lt;= 7">
                <xsl:value-of select="'textRed'"/>
            </xsl:when>
            <xsl:when
                    test="not(number(substring($color, 2, 1)) &lt;= 7) and number(substring($color, 4, 1)) &lt;= 7 and not(number(substring($color, 6, 1)) &lt;= 7)">
                <xsl:value-of select="'textMagenta'"/>
            </xsl:when>
            <xsl:when
                    test="not(number(substring($color, 2, 1)) &lt;= 7) and not(number(substring($color, 4, 1)) &lt;= 7) and number(substring($color, 6, 1)) &lt;= 7">
                <xsl:value-of select="'textYellow'"/>
            </xsl:when>
            <xsl:when
                    test="not(number(substring($color, 2, 1)) &lt;= 7) and not(number(substring($color, 4, 1)) &lt;= 7) and not(number(substring($color, 6, 1)) &lt;= 7)">
