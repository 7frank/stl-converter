...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="/">
        <xsl:choose>
            ...
            <xsl:when test="$offsetInFrames and (string-length(translate($offsetInFrames, ':', '')) != 8)">
                ...
                <xsl:message terminate="yes">
                    The value for the parameter offsetInFrames does not have the necessary format of 'hh:mm:ss:ff'. The
                    value for hours shall be a number between 00 and 23, minutes and
                    and seconds shall be between 00 and 59 and with a framerate of 25 the framerate the frame part shall
                    be between 00 and 24 (begin and end of the intervals are included).
                </xsl:message>
            </xsl:when>
            <xsl:when test="$offsetInFrames and 
                ...
                <xsl:message terminate="yes">
                    The value for the parameter offsetInFrames does not have the necessary format of "hh:mm:ss:ff". The
                    value for hours shall be a number between 00 and 23, minutes and
                    and seconds shall be between 00 and 59 and with a framerate of 25 the framerate shall be between 00
                    and 24 (begin and end of the intervals are included).
                </xsl:message>
            </xsl:when>
            <xsl:when test="$offsetInSeconds and (translate($offsetInSeconds, '0123456789-', '') != '')">
                <!--@ Interrupt transformation if content contains non-numerical values -->
                <xsl:message terminate="yes">
                    The value of the parameter offsetInSeconds contains non-numerical values. Fractions are not
                    supported for the parameter offsetInSeconds.
                </xsl:message>
            </xsl:when>
        </xsl:choose>
            ...
            <tt:styling>
                ...
                <tt:style
                    ...
                    <xsl:attribute name="tts:fontFamily">
                        <xsl:choose>
                            <xsl:when
                                    test="tt:styling/tt:style[@xml:id = 'defaultStyle'] and tt:styling/tt:style[@xml:id = 'defaultStyle']/@tts:fontFamily">
    ...
    <xsl:template match="@tts:backgroundColor | @tts:color">
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="translate(normalize-space(.), $smallcase, $uppercase) = 'FUCHSIA' or translate(normalize-space(.), $smallcase, $uppercase) = 'MAGENTA'">
            ...
            <xsl:when
                    test="translate(normalize-space(.), $smallcase, $uppercase) = 'AQUA' or translate(normalize-space(.), $smallcase, $uppercase) = 'CYAN'">
        ...
        <xsl:choose>
            <xsl:when test="$legacyTimeBase = 'smpte'">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:message terminate="yes">
                            The begin attribute should use a valid smpte timeformat if 'smpte' is given as
                            timeCodeFormat.
                        </xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:when test="$legacyTimeBase = 'media'">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:message terminate="yes">
                            The begin attribute should use a valid media timeformat if 'media' is given as
                            timeCodeFormat.
                        </xsl:message>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="timestampSummedUp">
                    <xsl:choose>
                        ...
                        <xsl:when test="$legacyTimeBase='smpte'">
                            <xsl:value-of
                                    select="($hours * 3600 + $minutes * 60 + $seconds + ($frames div $frameRate)) * 1000"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:variable>
                ...
                <xsl:variable name="mediaSeconds">
                    <xsl:value-of
                            select="floor(((($timestampSummedUp - $offsetSummedUp) mod 3600000) mod 60000) div 1000)"/>
                </xsl:variable>
                <xsl:variable name="mediaFrames">
                    <xsl:value-of
                            select="floor((($timestampSummedUp - $offsetSummedUp) mod 3600000) mod 60000) mod 1000"/>
                </xsl:variable>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$legacyTimeBase = 'media'">
                        <xsl:value-of
                                select="concat($outputHours, ':', $outputMinutes, ':', $outputSeconds, '.', $frames)"/>
                    </xsl:when>
